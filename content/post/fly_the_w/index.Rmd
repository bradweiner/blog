...
The reason is that I wanted to bot to represent a close to real-time version of the actual flag, which is already flying by the time Wrigleyville denizens crack open a post-game Old Style ^[Baseball fans will note that the W Flag only flies after home games. With rare exceptions, opposing teams not been convinced to celebrate Cubs wins in _their_ home ballparks].
...
Well, what you don't see is that each time the bot Tweets, it is uploading copy of the media image to Twitter and pointing to a _unique_ link. In short, I had unwittingly created a way to have multiple _unique_ tweets per day.
...
Since this error^[Baseball pun intended] occurred on Opening Day, 2020, I foolishly hadn't updated the code from the earlier year. It was pointed to `already_tweeted_2019` rather than `already_tweeted_2020`. Since the unique game ids are coded based on date, there was no universe where an id in the 2020 season would have already been posted in 2019.
...
It also gave me the idea to give this bot its own hashtag `#FlyTheWBot` which makes it discoverable for people who care, yet mute-able for people want to ignore these Tweets while still following my account.
