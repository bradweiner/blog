...
summary: ''
authors: []
lastmod: '2021-04-05T08:53:42-06:00'
featured: yes
image:
  placement: 1
...
encourage folks to find a problem to solve that is *interesting* and
*unrelated to work*. In other words, a side project.

The rationale for this advice is simple. Learning something new is
hard. Therefore, you should tackle problems that bring as much joy while getting 
you out of entangled email or late-night Slack boondoggles.
...
(and likely is for people who *already* know how), I wanted to gain some
...
Here are some lessons I've learned a long the way.
...
This had the same basic process (API call, data pull, aggregation and posting),
but it was a manual process requiring my sitting in front of my computer every night. Given the
...
With the Fly The W Bot, I wasn't committed to watching every game all
season, and had no plans to manually run the script each time the final out was recorded.
So, I created a production pipeline using Amazon Web Services (AWS).
...
**An R script that:**
  ...
  :white_check_mark: Creates a data frame with the fields of interest (home score, away score, winning picther, losing pitcher, date, time, venue)  
  :white_check_mark: Utilizes the `rtweet` package to post a Tweet IF the game is complete AND the Cubs won  
  :white_check_mark: Logs a copy of the data frame along with a time stamp for logging/error checking purposes.
...
This basic setup will continually re-run the same R script, repeatedly, every two minutes, during the baseball season as long as the EC2 instance is on. 

### Lesson #2: Optimize everything, including costs

After getting the basic setup deployed and working, the next question was how to I minimize the costs?
...
For context, a full MLB.tv digital package costs $129. That's to watch every single game from _almost_ every team, all season long. Also, it's possible that a fully headless/serverless mechanism could potentially run for free.   

Ultimately, the rationale for cost optimization is to replicate choices that exist in the actual business world. Even though I might personally value this bot to the tune of \$70 annually, a future CEO or CTO might have a different take when compute and storage costs put real strains on budgets or organizational ambitions. If this fictional CTO were to ask me to shave 10% of our AWS budget, I wanted to experience this scenario at the penny level rather than the  "burn rate" level.

To minimize costs I sought to programmatically narrow the window of time when the EC2 instance was running with the following steps
...
Step #1 isn't a fully straightforward process. During the upcoming season, Cubs games start at 28 different times across four American time zones. Furthermore, the un-timed nature of baseball leads to highly variant durations. 
...
Naturally, someone might ask why not just spin up the server every morning at, say, 2am when it is highly likely that the preceding day's game has concluded? This would be standardized and very cheap.

The reason is that I wanted to bot to represent a close to real-time version of the actual flag, which is already flying by the time Wrigleyville denizens crack open a post-game Old Style ^[Baseball fans will note that the W Flag only flies afer home games. With rare exceptions, opposing teams not been convinced to celebrate Cubs wins in _their_ home ballparks].
...
**This required creating:**
  ...
  :white_check_mark: Cloudwatch event triggers to fire the function at 5pm CST every day, and one to fire the stop function at 3am CST every day. 
  
This range, reduces server spend time from 24 hours to a maximum of 10. It is possible that this range will delay Tweets from quickly-played, early starting games, but is unlikely to shut down before the completion a late night, extra inning barnburner.
...
The real server wasteage occurs when the instance runs past the completion of a game. Having an outside boundary is acceptable to ensure that the bot is awake when the game end but it is clearly sub-optimal assume ten hours of run time when the average game ends after three. To alleviate this, I created some logic to shut down the server once a game has wrapped up. 
...
and personal work so far, has been totally do-able within the R ecosystem. I gotten deeper 
...
Indeed much of the AWS ecosystem has a Pythonic version including some key features
in Lambda (serverless functions) and Glue (scaled ETL). 

Additionally, basic connections to the AWS ecosystem with Python's [boto3](https://aws.amazon.com/sdk-for-python/)
...
Every project, whether it's data related or not, has a limitless number of ways where it could go sideways. Some of these start as unknown unknowns, but some are legitimate oopsies, that exist fully between the keyboard and chair.

In the spirit, of [Data Mishaps Night](https://datamishapsnight.com/) and to help aspiring data folks alleviate some imposter syndrome, I believe it is best practice to candidly and transparently expose a few blunders.

#### That time Fly the W Became Hal 

Since I planned to publish this to my own Twitter account, the biggest concern was that the bot would (somehow) take on a life of its own and spam my existing followers.
...
With this approach, the Tweets were unique, thereby clearing the bar to get posted but without risking any more than a single Tweet per day.

Then, in a legitimate exercise of unintended consequences, I figured I could make the Tweet prettier by adding a W Flag Graphic. Thanks to the incredible `rtweet` package by Michael W. Kearney, programmatically adding media to a post was super easy.  An example of the prettified Tweet looks like this:
...
Well, what you don't see is that each time the bot Tweets, it is uploading copy of the media image to Twitter, and pointing the reference to a _unique_ link. In short, I had unwittingly created a way to have multiple _unique_ tweets per day.
...
Since this error^[Baseball pun intended] occured on Opening Day, 2020, I foolishly hadn't updated the code from the earlier year. So it was pointed to `already_tweeted_2019` rather than `already_tweeted_2020`. Since the unique game ids are coded based on date, there was no universe where an id in the 2020 season would have already been posted in 2019.

So, what happened? 

On Opening Day (which was in July due to the COVID-19 pandemic) the Cubs beat the Brewers 3-0. When the final out was recorded, the bot^[Let's be real here, the bot was written by a human, namely me] created a unique Tweet with a unique image URL that would never be duplicated. And, since the unique game ID for 2020 wasn't found in the _previous_ year's set of IDs, it just kept on Tweeting. And Tweeting. And Tweeting. 

Every two minutes until I was graciously notified that the script had lost its mind and that I would be subsequently muted. Here is the offending Tweet.
...
The main lesson learned here was that even under circumstances when there are technical guardrails in place, humans still err. 

I don't want to confuse this small-time, two-bit, silly Twitter bot with something meaningful or valuable. But imagine a business process that delvers real value to your users. It stands to reason that you could save some time, resources, embarrassment, or catastrophe by implementing code reviews, unit tests, a sandbox development environment, and some official way to move from dev to prod.

#### Football in the Groin

Beyond the previous coding blunder that caused a small, but real error in a production environment, there are other blunders that are worth sharing for their pure comedic gold.

As an initial pass, this was built entirely using the AWS UI in a web browser. This involved logging in to AWS, starting the instance and hot coding any updates into the live environment. 
...
I created a paradoxical product where firing up the system initiated a process that shut down the system. 
...
There were numerous ways out of this mess, some clunkier than others:
...
It's important to listen and make sure that your product (even silly ones like this) are meeting your end-user's needs.

A few pieces of feedback I have gotten:

1. The Tweet itself needs to be more informative. At first it was just a "W" with a date which lacked information richness relative to 280 character limit in Twitter. Over time, I added scores, the venue, and the winning and losing pitchers with more to come on the punch list.
...
It was recursive, and annoying. Someone reached out and politely applauded the project but indicated it wasn't helpful to have an army of self-perpetuating bots Retweeting one another. This was great feedback which promoted me to remove the #rstats hashtag for good. 

It also gave me the idea to give this bot it's own hashtag "#FlyTheWBot" which makes it discoverable for people who care, yet mute-able for people want to ignore these Tweets while still following my account.

3. On opening day 2020, when everything went awry, it wasn't long before someone pointed that out to me. Again, this is a low-stakes, silly Twitter bot, but in the real world, this would be the "Your system is broken cancel your weekend" kind of notification. This "feedback" is the kind that can't go ignored, but it is a reminder that end users or clients are often the first to notice when something is down. Have a process in place to quickly discover outages and a rapid response plan to correct them. In my case, had it gone on much longer, I am pretty sure Twitter would have shut down the account.
