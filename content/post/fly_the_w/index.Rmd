...
### Lesson #7: Know When to Fold 'Em

Sometimes the juice just isn't worth the squeeze and it's worth knowing when it's time to pack up. After Elon Musk took over Twitter in October, 2022, there were immediate and frustrating changes to the way that Twitter behaved. Most critcally, the API endpoints changed, along with the rules for running automated accounts. These changes, caused breakage in the underlying code, especially since the API wrappter R package {rtweet} was not being actively maintained. All of this led me to realize that as much fun as this project has been, it is no longer worth maintaining. 

So, it is time to shut it down. The Twitter @flythewbot Twitter (now, X, which is so stupid) has Tweeted it's last Cubs win.

Thanks for hanging out for the journey. Now, I need to find some other project to take on...
...
* Projects eventually run their course. Know when it is time to cut bait, when when it is time to keep fishing.
